...
if anno1 == anno2:
    totale_anno = 366 if anno1%400 == 0 and anno1%4 == 0 else 365
    if(mese1 > 1):
        totale_anno = totale_anno - gennaio
    if(mese1 > 2 or mese2 < 2):
        if(anno1%400 == 0 and anno1%4 == 0):
            totale_anno = totale_anno - 29
        else:
            totale_anno = totale_anno - febbraio
    if(mese1 > 3 or mese2 < 3):
        totale_anno = totale_anno - marzo
    if(mese1 > 4 or mese2 < 4):
        totale_anno = totale_anno - aprile
    if(mese1 > 5 or mese2 < 5):
        totale_anno = totale_anno - maggio
    if(mese1 > 6 or mese2 < 6):
        totale_anno = totale_anno - giugno
    if(mese1 > 7 or mese2 < 7):
        totale_anno = totale_anno - luglio
    if(mese1 > 8 or mese2 < 8):
        totale_anno = totale_anno - agosto
    if(mese1 > 9 or mese2 < 9):
        totale_anno = totale_anno - settembre
    if(mese1 > 10 or mese2 < 10):
        totale_anno = totale_anno - ottobre
    if(mese1 > 11 or mese2 < 11):
        totale_anno = totale_anno - novembre
    if(mese2 < 12):
        totale_anno = totale_anno - dicembre
else:
